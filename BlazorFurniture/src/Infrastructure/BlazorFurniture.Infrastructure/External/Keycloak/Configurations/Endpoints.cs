...
internal sealed class Endpoints( string Realm )
{
    ...
    public string AuthzPermission() => $"{BaseRealmUrl}/authz/protection/permission";

    // Users
    public string Users() => $"{AdminRestApiBaseUrl}/users";
    public string UsersExtension() => $"{Users()}-extension";
    public string UsersCount() => $"{UsersExtension()}/count";
    ...
    public string GroupMemberByIdRole( Guid groupId, Guid userId, Guid roleId ) => $"{GroupMembersExtension(groupId)}/{userId}/roles/{roleId}";
}
